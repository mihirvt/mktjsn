...
class VoicemakerTTSService(TTSService):
    ...
    async def _connect(self) -> AsyncGenerator[Frame, None]:
        ...
        try:
            ...
            yield ErrorFrame(f"Voicemaker TTS Connection Error: {e}")
    ...
    async def run_tts(
        ...
        # Connect if needed
        connect_stream = self._connect()
        async for frame in connect_stream:
        ...
        if self._ws is None or self._ws.closed:
            ...
            yield ErrorFrame("Voicemaker TTS: WebSocket not connected")
            return
        ...
        # Use simple Engine instead of ProEngine in some mappings, but keep both for safety if uncertain
        if p.pro_engine is not None:
            payload["Engine"] = p.pro_engine
        ...
        try:
            ...
            yield ErrorFrame(f"Voicemaker TTS send error: {e}")
            return
        ...
        try:
            while self._ws and not self._ws.closed:
                ...
                if msg.type == aiohttp.WSMsgType.TEXT:
                    ...
                    if not data.get("success", True):
                        ...
                        yield ErrorFrame(f"Voicemaker TTS Error: {err_msg}")
                        break
                    ...
                    if is_final:
                        if mp3_buffer:
                            # Whole MP3 accumulated — now decode to PCM and yield
                            pcm_bytes = _decode_mp3_to_pcm(mp3_buffer, self._sample_rate)
                            ...
                            if pcm_bytes:
                                ...
                                yield frame
                                logger.debug(
                                    f"Voicemaker: yielded {len(pcm_bytes)} PCM bytes "
            ...
            logger.error(f"Voicemaker TTS: stream error: {e}", exc_info=True)
            yield ErrorFrame(f"Voicemaker TTS Stream Error: {e}")
