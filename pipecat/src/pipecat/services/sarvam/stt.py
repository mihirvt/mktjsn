            ...
            if isinstance(frame, VADUserStartedSpeakingFrame):
                ...
                if self._socket_client:
                    try:
                        ...
                        logger.error(f"Error flushing Sarvam STT WebSocket: {e}")
    ...
    async def run_stt(self, audio: bytes) -> AsyncGenerator[Frame, None]:
        ...
        if not self._socket_client:
            ...
            try:
                ...
                logger.error(f"Failed to reconnect Sarvam STT: {e}")
                yield None
                return
            
            # If still not connected after attempting, bail out gracefully
            if not self._socket_client:
                yield None
                return
